...
public:
	void Switching_is_Wheathering() { m_is_Wheathering = m_is_Wheathering == true ? false : true; }
	int const Get_Wheather_Type() { return m_Wheather_Type; }
...
public:
	...
	void Floor_Initialize(const int& width, const int& height, const int& piece_width_size, const int& piece_height_size)
	{
		...
		for (int i = -width / 2; i <= width / 2; ++i)
		{
			for (int j = -height / 2; j <= height / 2; ++j)
			{
				...
				temp_Material.Ambient[0] = 0.6f; temp_Material.Ambient[1] = 0.6f; temp_Material.Ambient[2] = 0.6f; temp_Material.Ambient[3] = 1.0f;
	...
	virtual void Draw()
	{
		...
		if (m_is_Wheathering)
		{
			...
			while (m_Wheather_Particles_Coord_iter != m_Wheather_Particles_Coord.end())
			{
				...
				switch (m_Wheather_Type)
				{
				case WHEATHER_TYPE_SNOW:
					glutSolidSphere(OBJECT_SIZE * 0.05f, 5, 5);
					break;

				case WHEATHER_TYPE_RAIN:
					glBegin(GL_LINES);
					glVertex3f(0.0f, OBJECT_SIZE * 0.05f, 0.0f);
					glVertex3f(0.0f, -OBJECT_SIZE * 0.05f, 0.0f);
					glEnd();
					break;
				}
	...
	void Wheather_Initialize(const int& wheather_type, const int& particle_count, const bool& using_material)
	{
		...
		m_Wheather_Particles_Coord.clear();
		m_Wheather_Particles_Matirial.clear();
		m_Wheather_Particles_Speed.clear();
		...
		for (int i = 0; i < particle_count; ++i)
		{
			...
			{
			case WHEATHER_TYPE_SNOW:
				m_Wheather_Particles_Speed.push_back(m_WP_Random_Speed(m_Random_SEED)); break;
			case WHEATHER_TYPE_RAIN:
				m_Wheather_Particles_Speed.push_back(m_WP_Random_Speed(m_Random_SEED) * 5.0f); break;
			}
		...
		else
		{
			...
			case WHEATHER_TYPE_RAIN:
				m_Wheather_Particles_Color.R = 0.2f; m_Wheather_Particles_Color.G = 0.2f; m_Wheather_Particles_Color.B = 0.6f; m_Wheather_Particles_Color.A = 1.0f; break;
				...
				if (m_Wheather_Particles_Coord_iter->y < 0.0f)
				{
					...
					case WHEATHER_TYPE_SNOW:
						...
						*m_Wheather_Particles_Speed_iter = m_WP_Random_Speed(m_Random_SEED);
						break;
					case WHEATHER_TYPE_RAIN:
						*m_Wheather_Particles_Speed_iter = m_WP_Random_Speed(m_Random_SEED) * 5.0f;
						break;
					}
