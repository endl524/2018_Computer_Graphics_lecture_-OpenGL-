...
protected: // 멤버변수 선언구역
	...
	bool m_is_Normal_On = true;
	
	float m_Sphere_Rot_Value = 0.0f;

	int m_Snow_Index[2] = { 0,0 };
	bool m_is_Touched_Floor = false;
	float m_Snow_Falling_Speed = 0.0f;
	bool m_is_Snowing = true;
...
public: // Getter, Setter 정의구역
	...
	{
		m_Diffuse_Color[0] += 0.1f * sign; m_Diffuse_Color[1] += 0.1f * sign; m_Diffuse_Color[2] += 0.1f * sign;
	}
	...
	{
		m_Specular_Color[0] += 0.1f * sign; m_Specular_Color[1] += 0.1f * sign; m_Specular_Color[2] += 0.1f * sign;
	}

	void Set_is_Normal_On(const bool& b) { m_is_Normal_On = b; }
	bool const Get_is_Normal_On() { return m_is_Normal_On; }

	void Set_Snow_Index(const int& x, const int& y) { m_Snow_Index[0] = x; m_Snow_Index[1] = y; }
	int Get_Snow_Index_X() { return m_Snow_Index[0]; }
	int Get_Snow_Index_Y() { return m_Snow_Index[1]; }
	void Set_Snow_Falling_Speed(const float& v) { m_Snow_Falling_Speed = v; }
	void Set_is_Touched_Floor(const bool& b) { m_is_Touched_Floor = b; }
	bool const Get_is_Touched_Floor() { return m_is_Touched_Floor; }
	void Set_is_Snowing(const bool& b) { m_is_Snowing = b; }
	bool const Get_is_Snowing() { return m_is_Snowing; }
	void Switch_Snowing() { m_is_Snowing = m_is_Snowing == true ? false : true; }
...
public: // public 메소드 정의구역
	...
	void Normal_On_Switch()
	{
		m_is_Normal_On = m_is_Normal_On == true ? false : true;
	}
