...
private: // 멤버변수 선언구역
	...
	int m_Shape_Type = SHAPE_TYPE_RECTANGLE;
...
public: // public 메소드 정의구역
	...
	Object* Make_Object(const GLenum type, vector<Vertex2D>& v_vertexs, const float pos_x, const float pos_y, const float size_x, const float size_y);
	void Set_Rectangle(Vertex2D& temp_vertex, vector<Vertex2D>& v_vertexs, float pos_x, float pos_y, const float size_x, const float size_y);
	void Set_Triangle(Vertex2D& temp_vertex, vector<Vertex2D>& v_vertexs, const float pos_x, const float pos_y, const float size_x, const float size_y);
