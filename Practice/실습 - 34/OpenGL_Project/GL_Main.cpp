...
void GL_Main::Reset(const bool& is_Reset)
{
	...
	if (m_Particle[0] != nullptr) { for (int i = 0; i < 5; ++i) m_Particle[i] = nullptr; }
...
int GL_Main::Build_Object(const int& type)
{
	...
	case PYRAMID_OBJECT:
		...
		reinterpret_cast<Object*>(temp_Pyramid)->Set_Material_Ambient_Color(0.15f, 0.15f, 0.15f, 1.0f);
	...
	case PARTICLE_OBJECT:
		temp_Particle = new Particle_Object();
		temp_Object = reinterpret_cast<Object*>(temp_Particle);
		temp_Object->Set_Object_Type(PARTICLE_OBJECT);
		break;
	}
