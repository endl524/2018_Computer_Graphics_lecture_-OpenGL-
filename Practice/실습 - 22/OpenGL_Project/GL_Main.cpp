...
void GL_Main::Init_Objects()
{
	...
	m_instance->Object_Activate(m_instance->Build_Object(AIRPLANE_OBJECT)); // Sphere
	for (int i = 0; i < 4; ++i)
	{
		m_instance->Insert_Object(m_instance->m_Airplane->Get_Fog_Object(i));
		m_instance->Object_Activate(m_instance->m_Airplane->Get_Fog_Object(i)->Get_Index());
	}
	glutTimerFunc(OBJECT_TIMER_ELAPSED_TIME * 20, Start_Fog_Setting_Timer, 0);
...
void GL_Main::Key_Manager(unsigned char key, int x, int y)
{
	...
	// Roll
	case 'u': m_instance->m_Airplane->Roll(-10.0f); break;
	case 'U': m_instance->m_Airplane->Roll(10.0f); break;
...
int GL_Main::Build_Object(const int& type)
{
	...
	case AIRPLANE_OBJECT:
		m_Airplane = new Airplane();
		temp_Object = reinterpret_cast<Object*>(m_Airplane);
		break;
	}
...
void GL_Main::Start_Fog_Setting_Timer(const int object_num)
{
	if (m_instance->m_Airplane)
	{
		m_instance->m_Airplane->Set_Fog_Objects();
		glutTimerFunc(OBJECT_TIMER_ELAPSED_TIME * 20, Start_Fog_Setting_Timer, object_num);
	}
}
