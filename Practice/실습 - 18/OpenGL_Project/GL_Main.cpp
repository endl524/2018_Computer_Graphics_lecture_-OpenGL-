...
void GL_Main::Key_Manager(unsigned char key, int x, int y)
{
	...
	case 'i': m_instance->m_Camera->Forward_Moving(-CAMERA_MOVE_VALUE); break;

	case 'k': m_instance->m_Camera->Forward_Moving(CAMERA_MOVE_VALUE); break;

	case '+': m_instance->m_Camera->Set_Rotation_Value(1, 3.0f); break;
	case '-': m_instance->m_Camera->Set_Rotation_Value(1, -3.0f); break;
