...
public:
	...
	void Set_Material_Ambient_Color(const float& r, const float& g, const float& b, const float& a)
	{
		if (m_Light_Number >= GL_LIGHT0 && m_Light_Number <= GL_LIGHT7)
		{
			...
			glLightfv(m_Light_Number, GL_AMBIENT, m_Material.Ambient);
		}
	}
	...
	void Set_Material_Diffuse_Color(const float& r, const float& g, const float& b, const float& a)
	{
		if (m_Light_Number >= GL_LIGHT0 && m_Light_Number <= GL_LIGHT7)
		{
			...
			glLightfv(m_Light_Number, GL_DIFFUSE, m_Material.Diffuse);
		}
	}
	...
	void Set_Material_Specular_Color(const float& r, const float& g, const float& b, const float& a)
	{
		if (m_Light_Number >= GL_LIGHT0 && m_Light_Number <= GL_LIGHT7)
		{
			...
			glLightfv(m_Light_Number, GL_SPECULAR, m_Material.Specular);
		}
	}
	...
	virtual void Control_Ambient_Color(const int& sign)
	{
		if (m_Light_Number >= GL_LIGHT0 && m_Light_Number <= GL_LIGHT7)
		{
			m_Material.Ambient[0] += 0.1f * sign;
			m_Material.Ambient[1] += 0.1f * sign;
			m_Material.Ambient[2] += 0.1f * sign;
			glLightfv(m_Light_Number, GL_DIFFUSE, m_Material.Ambient);
		}
	}
	...
	virtual void Control_Diffuse_Color(const int& sign)
	{
		if (m_Light_Number >= GL_LIGHT0 && m_Light_Number <= GL_LIGHT7)
		{
			m_Material.Diffuse[0] += 0.1f * sign;
			m_Material.Diffuse[1] += 0.1f * sign;
			m_Material.Diffuse[2] += 0.1f * sign;
			glLightfv(m_Light_Number, GL_DIFFUSE, m_Material.Diffuse);
		}
	}
	...
	virtual void Control_Specular_Color(const int& sign)
	{
		if (m_Light_Number >= GL_LIGHT0 && m_Light_Number <= GL_LIGHT7)
		{
			m_Material.Specular[0] += 0.1f * sign;
			m_Material.Specular[1] += 0.1f * sign;
			m_Material.Specular[2] += 0.1f * sign;
			glLightfv(m_Light_Number, GL_SPECULAR, m_Material.Specular);
		}
	}
