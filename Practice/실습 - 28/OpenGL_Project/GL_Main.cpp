...
void GL_Main::Mouse_Manager(int button, int state, int x, int y)
{
	...
	case GLUT_LEFT_BUTTON:
		...
		{
			m_instance->m_Object->Insert_Control_Point(x - INIT_WINDOW_SIZE_X * 0.5f, INIT_WINDOW_SIZE_Y * 0.5f - y, 0.0f);
	...
	case GLUT_RIGHT_BUTTON:
		...
		{
			m_instance->m_Selected_Point_index = -1;
...
{
	if (m_instance->m_is_Right_Clicked && m_instance->m_Selected_Point_index != -1)
	{
...
{ 
	...
	glOrtho(-w * 0.5f, w * 0.5f, -h * 0.5f, h * 0.5f, -400.0f, 1000.0f);
}
