...
void Camera::Look()
{
	if (m_Target != nullptr)
	{
		if (m_is_FPS)
		{
			if (m_Target->Get_Object_Type() == ROLLER_COASTER)
			{
				m_Position_Vector[0] = m_Target->Get_Head_Position_X();
				m_Position_Vector[1] = m_Target->Get_Head_Position_Y();
				m_Position_Vector[2] = m_Target->Get_Head_Position_Z();
				m_Look_Vector[0] = m_Position_Vector[0] + m_Target->Get_Look_Vector(0) * 2.0f;
				m_Look_Vector[1] = m_Position_Vector[1] + m_Target->Get_Look_Vector(1) * 2.0f;
				m_Look_Vector[2] = m_Position_Vector[2] + m_Target->Get_Look_Vector(2) * 2.0f;
			}
			else 
			{
				m_Position_Vector[0] = m_Target->Get_Head_Position_X() + m_Target->Get_Forward_Vector_X() * 10.0f;
				m_Position_Vector[1] = m_Target->Get_Head_Position_Y() + m_Target->Get_Forward_Vector_Y() * 10.0f;
		...
		{
			m_Position_Vector[0] = m_Target->Get_Head_Position_X() + m_Target->Get_Forward_Vector_X() * -100.0f + m_Target->Get_Up_Vector_X() * 100.0f;
			m_Position_Vector[1] = m_Target->Get_Head_Position_Y() + m_Target->Get_Forward_Vector_Y() * -100.0f + m_Target->Get_Up_Vector_Y() * 100.0f;
			m_Position_Vector[2] = m_Target->Get_Head_Position_Z() + m_Target->Get_Forward_Vector_Z() * -100.0f + m_Target->Get_Up_Vector_Z() * 100.0f;
